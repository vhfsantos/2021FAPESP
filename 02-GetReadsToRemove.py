...
def GetReadsToRemove(bc):
        with open('Stats/barcode'+bc+'.fixedMGEs.txt','r') as fxd:
                fixed = []
                for line in fxd.readlines():
                        fixed.append(line)
        with open('Stats/barcode'+bc+'.optionalMGEs.txt','r') as opt:
                optional = []
                for line in opt.readlines():
                        optional.append(line)
        with open('Stats/barcode'+bc+'.all-reads.txt', 'r') as allids:
                allreads = []
        ...
        plt.savefig('Stats/bc'+bc+'.svg')
        print('Writing txt file')
        ToRemove = set(optional).difference(set(fixed))
        with open('Stats/barcode'+bc+'.to-remove.txt','w') as file:
